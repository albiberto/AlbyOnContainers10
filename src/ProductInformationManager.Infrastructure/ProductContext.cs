    ...
    IEnumerable<IInterceptor> interceptors) : DbContext(options), IUnitOfWork
{
    ...
    public DbSet<CategoryDescriptionRule> CategoryDescriptionRules => Set<CategoryDescriptionRule>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        // === CategoryDescriptionRule (Tabella di Join Esplicita con Logica/Payload) ===
        modelBuilder.Entity<CategoryDescriptionRule>(entity =>
        {
            // Chiave primaria composta dai due ID
            entity.HasKey(r => new { r.CategoryId, r.DescriptionTypeId });

            entity.HasOne(r => r.Category)
                  .WithMany(c => c.DescriptionRules)
                  .HasForeignKey(r => r.CategoryId)
                  .OnDelete(DeleteBehavior.Cascade);

            entity.HasOne(r => r.DescriptionType)
                  .WithMany(d => d.CategoryRules)
                  .HasForeignKey(r => r.DescriptionTypeId)
                  .OnDelete(DeleteBehavior.Restrict); // Non eliminare il Tipo se è usato in una regola
        });
