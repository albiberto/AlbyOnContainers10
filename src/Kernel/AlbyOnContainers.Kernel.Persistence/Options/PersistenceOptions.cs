...
public sealed record PersistenceOptions : KernelOptions<PersistenceOptions>
{
    [Required(ErrorMessage = "The metric prefix is required to ensure proper OpenTelemetry namespace grouping."), RegularExpression(@"^[a-zA-Z_][a-zA-Z0-9_]*$", ErrorMessage = "The metric prefix must contain only alphanumeric characters or underscores, and cannot start with a number.")]
