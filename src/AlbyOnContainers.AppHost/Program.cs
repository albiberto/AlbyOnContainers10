...
// Keycloak — non serve assegnarlo a una variabile se non viene referenziato
...
// Tempo — espone solo HTTP :3200 verso l'host per Grafana
// Il gRPC :4317 è SOLO interno (non esposto sull'host, evita conflitto col collector)
var tempo = builder.AddContainer("tempo", "grafana/tempo", "latest")
    ...
    // RIMOSSO: .WithEndpoint(port: 4317, ...) — conflitto con collector sull'host
...
// OTel Collector — unico owner della porta 4317 sull'host
...
var otelServiceName = builder.Configuration["Otel:ServiceName"] ?? "productdatamanager";
var otelNamespace   = builder.Configuration["Otel:Namespace"]   ?? "albyon";
var otelEnv         = builder.Configuration["Otel:Environment"] ?? "development";
...
builder.AddProject<Projects.ProductInformationManager_Web>("productdatamanager")
    ...
    .WaitFor(messaging)
    .WaitFor(redis)
    .WaitFor(collector)                    // FIX: aspetta il collector prima di partire
    ...
    .WithEnvironment("OTEL_RESOURCE_ATTRIBUTES",
        $"service.namespace={otelNamespace},deployment.environment={otelEnv}")
