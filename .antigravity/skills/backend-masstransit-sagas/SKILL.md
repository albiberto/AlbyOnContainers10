...
5. **STATE PERSISTENCE:** The Saga State class must inherit from `SagaStateMachineInstance`. When configuring EF Core for the Saga, ensure the `CurrentState` is persisted as a string or integer, and map the `CorrelationId` as the primary key.

## Execution
When implementing a SAGA:
1. Define the State class (`SagaStateMachineInstance`).
